    ...
    <>
      ...
      {jsonData.map((data, index) => (
        <Link to="/calendar" key={index}>
          ...
          <button className="px-4 py-2 w-full sm:w-96 bg-gradient-to-r from-amber-300 via-yellow-300 to-orange-500 rounded-full flex flex-row items-center justify-center mx-auto">
      ...
      <button className="px-4 py-2 w-full sm:w-96 bg-gradient-to-r from-amber-300 via-yellow-300 to-orange-500 rounded-full flex flex-row items-center justify-center mx-auto">
  <div className="text-sm lg:text-lg text-blue-500 font-semibold flex-shrink-0">இன்றைய நாள்</div>
  <div className="text-sm lg:text-2xl text-blue-500 mx-2">|</div>
  <div className="text-sm lg:text-lg text-blue-500 font-semibold flex-shrink-0">Today's Panchagam</div>
</button>
